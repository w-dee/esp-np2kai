...
DESCRIPTOR_PATH = ROOT / "tests/guest/np2video/golden.json"
...
def valid_log(descriptor: dict[str, object]) -> str:
    ...
    return "\n".join([
        ...
        "NP2VIDEO_FIXTURE scene_id={scene_id} fixture_sha256={fixture_sha256} image_bytes={image_size} partition=np2test".format(**descriptor),
        "NP2VIDEO_READY scene_id={scene_id} state={ready_state} generation=7 surface_update_sequence=3".format(**descriptor),
        "NP2VIDEO_FRAMEBUFFER scene_id={scene_id} width={width} height={height} bytes={visible_bytes} format={pixel_format} bpp={bpp} pitch={pitch} generation=7 surface_update_sequence=4 crc_algorithm={crc_algorithm} crc32={crc32} storage_external=1".format(**frame_descriptor),
...
def main() -> int:
    ...
    print(f"np2video ESP log validator negative tests: PASS ({len(cases)} cases)")
    return 0
