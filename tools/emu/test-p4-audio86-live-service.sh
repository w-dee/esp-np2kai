...
readonly RUN_ROOT="${P4_AUDIO86_LIVE_SERVICE_RUN_ROOT:-$(mktemp -d "${TMPDIR:-/tmp}/p4audio86-live-service.XXXXXX")}"
readonly BUILD_DIR="${RUN_ROOT}/build"
readonly MERGED_IMAGE="${RUN_ROOT}/p4-audio86-live-service.bin"
readonly LOG="${RUN_ROOT}/p4-audio86-live-service.log"
...
bash "${SCRIPT_DIR}/build-production.sh" \
    --variant p4-v3x --board generic --audio86-live-service \
...
python3 "${SCRIPT_DIR}/validate_p4_audio86_live_service_log.py" --log "${LOG}"
printf 'P4_AUDIO86_LIVE_SERVICE_RUN_ROOT=%s\n' "${RUN_ROOT}"
