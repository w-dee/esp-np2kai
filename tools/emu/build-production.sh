...
if (( benchmark_display_refresh_requested )) &&
   (( ! exact2x_internal_source_benchmark && ! ppa_pie_overlap_benchmark )); then
    printf 'ERROR: --benchmark-display-refresh requires --exact2x-internal-source-benchmark or --ppa-pie-overlap-benchmark\n' >&2
    exit 2
fi

if (( live_display_transform_isolated_benchmark || transform_isolated_compute_control_benchmark || transform_isolated_psram_read_control_benchmark || ppa_rotation_benchmark || ppa_internal_tile_benchmark || exact2x_scaler_benchmark || exact2x_internal_source_benchmark || ppa_pie_overlap_benchmark )) &&
...
if (( display_transform_diagnostic || live_display || live_display_motion_validation || live_display_benchmark ||
      ...
      ppa_pie_overlap_benchmark )); then
    transform_profile=1
fi
...
cmake_args=(
    ...
    -D "P4_NANO_LIVE_DISPLAY_TRANSFORM_ISOLATED_BENCHMARK_PROFILE=$((live_display_transform_isolated_benchmark || ppa_internal_tile_benchmark || exact2x_scaler_benchmark || exact2x_internal_source_benchmark || ppa_pie_overlap_benchmark))"
    -D "P4_NANO_LIVE_DISPLAY_TRANSFORM_ISOLATED_BENCHMARK_BOARD=$((live_display_transform_isolated_benchmark || ppa_internal_tile_benchmark || exact2x_scaler_benchmark || exact2x_internal_source_benchmark || ppa_pie_overlap_benchmark))"
    -D "P4_NANO_LIVE_DISPLAY_TRANSFORM_ISOLATED_BENCHMARK_VARIANT=${live_display_transform_isolated_benchmark_variant:-${ppa_internal_tile_benchmark_variant:-${exact2x_scaler_benchmark_variant:-${exact2x_internal_source_benchmark_variant:-${ppa_pie_overlap_benchmark_variant}}}}}"
...
if (( live_display_transform_isolated_benchmark || ppa_rotation_benchmark || ppa_internal_tile_benchmark || exact2x_scaler_benchmark || exact2x_internal_source_benchmark || ppa_pie_overlap_benchmark )); then
    export P4_NANO_LIVE_DISPLAY_TRANSFORM_ISOLATED_BENCHMARK_PROFILE=1
    export P4_NANO_LIVE_DISPLAY_TRANSFORM_ISOLATED_BENCHMARK_BOARD=1
    if (( ppa_rotation_benchmark )); then
        export P4_NANO_LIVE_DISPLAY_TRANSFORM_ISOLATED_BENCHMARK_VARIANT="${ppa_rotation_benchmark_variant}"
