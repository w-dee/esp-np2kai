...
if grep -Eq 'NP2REDUCED_RESULT=PASS\r?$' "${EMULATOR_LOG}"; then
    ...
    grep -Eq 'NP2FIXTURE_RESULT=PASS[[:space:]]*$' "${EMULATOR_LOG}" ||
        fail "raw fixture path did not pass"
    ...
    grep -Eq 'NP2REDUCED_STACK configured_bytes=[0-9]+ high_water_bytes=[0-9]+\r?$' \
