...
readonly RUN_ROOT="${P4_AUDIO86_RUN_ROOT:-$(mktemp -d "${TMPDIR:-/tmp}/p4audio86.XXXXXX")}"
readonly BUILD_DIR="${RUN_ROOT}/build"
readonly MERGED_IMAGE="${RUN_ROOT}/p4-audio86-merged.bin"
readonly LOG="${RUN_ROOT}/p4-audio86-esp-emu.log"
...
bash "${SCRIPT_DIR}/build-production.sh" \
    --variant p4-v3x --board generic --audio86-capacity \
...
set +e
timeout --foreground 60s "${ESP_EMU}" --chip esp32p4 --firmware "${MERGED_IMAGE}" \
    --exit-on 'AUDIO86_P4_EMU_SMOKE=PASS' --timeout 50s \
    --log-color never 2>&1 | tee "${LOG}"
status="${PIPESTATUS[0]}"
set -e
(( status == 0 )) || fail "esp-emu failed with status ${status}; log=${LOG}"
python3 "${SCRIPT_DIR}/validate_p4_audio86_capacity_log.py" --log "${LOG}" --smoke
printf 'P4_AUDIO86_EMU_RUN_ROOT=%s\n' "${RUN_ROOT}"
