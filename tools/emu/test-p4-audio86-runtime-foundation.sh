...
readonly RUN_ROOT="${P4_AUDIO86_RUNTIME_RUN_ROOT:-$(mktemp -d "${TMPDIR:-/tmp}/p4audio86-runtime.XXXXXX")}"
readonly BUILD_DIR="${RUN_ROOT}/build"
readonly MERGED_IMAGE="${RUN_ROOT}/p4-audio86-runtime-merged.bin"
readonly LOG="${RUN_ROOT}/p4-audio86-runtime-esp-emu.log"
...
python3 "${SCRIPT_DIR}/validate_p4_audio86_runtime_foundation_log.py" \
    --log "${LOG}"
printf 'P4_AUDIO86_RUNTIME_RUN_ROOT=%s\n' "${RUN_ROOT}"
