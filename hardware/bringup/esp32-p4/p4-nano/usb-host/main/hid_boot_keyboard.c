...
static void emit_event(hid_boot_keyboard_event_t *events, size_t capacity,
                       size_t *count, hid_boot_keyboard_event_kind_t kind,
    ...
    size_t event_capacity)
{
    ...
    for (size_t i = 0; i < HID_BOOT_KEYBOARD_KEY_COUNT; ++i) {
        if (hid_boot_keyboard_is_error_usage(keys[i])) {
            ...
            emit_event(events, event_capacity, &count,
                       HID_BOOT_EVENT_ERROR_USAGE, keys[i], modifiers);
        }
    }
    ...
    for (unsigned bit = 0; bit < 8U; ++bit) {
        ...
        if ((previous_modifiers & mask) == 0U && (modifiers & mask) != 0U) {
            emit_event(events, event_capacity, &count,
                       HID_BOOT_EVENT_MOD_PRESS, modifier_usage(bit), modifiers);
        } else if ((previous_modifiers & mask) != 0U && (modifiers & mask) == 0U) {
            emit_event(events, event_capacity, &count,
                       HID_BOOT_EVENT_MOD_RELEASE, modifier_usage(bit), modifiers);
        }
    }

    for (size_t i = 0; i < HID_BOOT_KEYBOARD_KEY_COUNT; ++i) {
        const uint8_t usage = state->keys[i];
        if (usage != 0U && !key_present(keys, usage)) {
            emit_event(events, event_capacity, &count,
                       HID_BOOT_EVENT_KEY_RELEASE, usage, modifiers);
        }
    }
    ...
    for (size_t i = 0; i < HID_BOOT_KEYBOARD_KEY_COUNT; ++i) {
        ...
        if (usage != 0U && !key_seen_before(keys, i, usage) &&
            ...
            emit_event(events, event_capacity, &count,
                       HID_BOOT_EVENT_KEY_PRESS, usage, modifiers);
        }
    }
