...
; NP2 video fixture Step 7A.3d: deterministic GDC stage-2 loader.
...
loader:
    ...
    ; BIOS RAM 0584h contains the full INT 1Bh drive selector. Only the
    ; documented 90h..93h family is accepted; AL is preserved for INT 1Bh.
    mov al, [0x0584]
    mov ah, al
    and al, 0xfc
    cmp al, 0x90
...
read_next_sector:
    ...
    mov ax, 0x2000
    mov es, ax
    ; The INT 1Bh 2HD MFM read function is 56h in the NP2kai BIOS model;
    ; its low-nibble operation is the required 16h read operation.
    mov ah, 0x56
