...
#elif defined(P4_NANO_AUDIO_I2S_OPNGEN_PROFILE)
...
extern "C" void app_main(void)
{
...
#if defined(P4_NANO_AUDIO86_REAL_GUEST_PROFILE)
    const esp_err_t profile_status = p4_nano_pc98_runtime::run_audio86_real_guest();
    std::printf("P4_NANO_AUDIO86_REAL_GUEST_STATUS=%s\n",
    ...
    if (profile_status != ESP_OK) {
        std::printf("P4_NANO_AUDIO86_CAPACITY_STATUS=FAIL err=%s\n",
                    esp_err_to_name(profile_status));
    }
    std::fflush(stdout);
    return;
#elif defined(P4_NANO_AUDIO_ONLY_BENCHMARK_PROFILE)
    ...
    return;
#elif defined(P4_NANO_AUDIO_I2S_OPNGEN_PROFILE)
    const esp_err_t audio_result = p4_nano_audio_i2s_opngen::run();
    std::printf("P4_AUDIO_I2S_OPNGEN_EXIT=%s\n",
                audio_result == ESP_OK ? "PASS" : "FAIL");
    std::fflush(stdout);
    return;
#elif defined(P4_NANO_AUDIO_I2S_TONE_PROFILE)
