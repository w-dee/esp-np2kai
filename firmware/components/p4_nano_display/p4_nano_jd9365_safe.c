...
/*
 * Production project-owned adaptation of Waveshare
 * esp_lcd_jd9365_10_1 version 1.0.4, upstream commit
 * e7721dd43e55cd6b10110543e3efa8dca8e3bfe4, from the original source path
 * esp_lcd_jd9365_10_1.c. The upstream constructor's hidden display-side
 * I2C/backlight block is intentionally removed. Backlight and power policy
 * is owned by the p4_nano_board component.
 *
 * The source-level SPDX metadata is preserved from the upstream adaptation.
 * Package-level metadata reports MIT separately; this implementation keeps
 * both facts for later licensing review rather than rewriting either one.
 */
...
esp_err_t p4_nano_esp_lcd_new_panel_jd9365_safe(const esp_lcd_panel_io_handle_t io, const esp_lcd_panel_dev_config_t *panel_dev_config,
