...
{
    std::array<char, MAX_PATH + 1> overlong{};
    overlong.fill('x');
    overlong.back() = '\0';
...
bool run_all_tests()
{
    ...
    if (!run_attach_tests() || !run_attr_tests() || !run_open_size_tests() ||
        !run_read_tests() || !run_seek_eof_tests() || !run_close_detach_tests() ||
        !hash_through_dosio(actual_sha256)) {
        return false;
    }
